...
export const createQuestion = async (req, res) => {
  try {
    ...
    const [result] = await db.execute('INSERT INTO Questions (title, content) VALUES (?, ?)', [title, content]);
...
export const updateQuestion = async (req, res) => {
    ...
    const { id } = req.params;
    const { title, content } = req.body;
    await db.execute('UPDATE Questions SET title = ?, content = ? WHERE id = ?', [title, content, id]);
